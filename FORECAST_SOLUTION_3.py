# streamlit_forecast_multi_fluid_report.py
...
st.title("Well Production Forecast — Oil, Gas, Condensate, Water")
...
def generate_forecasts(df, start_year=2025, end_year=2030, default_decline_rate=5):
    ...
    # Determine candidate column names for gas/gor/cgr
    ...
    df['Start Date'] = pd.to_datetime(df.get('Forecast_Start_Test'), format="%B, %Y", errors='coerce')
    ...
    for _, well in df.iterrows():
        ...
        n = len(dates)
        oil = np.zeros(n)
        gas_ag = np.zeros(n)    # associated gas from oil via GOR (MMscfd)
        gas_nag = np.zeros(n)   # non-associated gas (MMscfd)
        gas_total = np.zeros(n)
        cond = np.zeros(n)      # condensate bbl/d from NAG only (bbl/d)
        water = np.zeros(n)
        wor = np.zeros(n)
        ...
        # Convert daily rates to monthly volumes using per-well proddays
        ...
        for i, dt in enumerate(dates):
            rows.append({
                ...
                'Oil (bopd)': oil[i],
                'Gas (MMscfd)': gas_total[i],                 # total gas daily (AG + NAG)
    ...
    # --- Monthly/yearly summaries (sums of monthly volumes, averages of daily rates) ---
    monthly_well = (wf.groupby(['Month', 'Well Name'])
                    .agg(
                        ...
                        Condensate_Avg_bbl=('Condensate (bbl/d) (from NAG)', 'mean'),
    ...
    # Yearly summaries by Well and Field — totals are computed as sum of monthly volumes in the year
    yearly_well = (
        wf.groupby(['Year', 'Well Name'])
        .agg(
            Oil_Total_bbl=('Monthly Oil (bbl)', 'sum'),
            Oil_Avg_bopd=('Oil (bopd)', 'mean'),  # avg daily rate in that year
            Gas_Total_mmscf=('Monthly Gas (MMscf)', 'sum'),
            Gas_Avg_mmscfpd=('Gas (MMscfd)', 'mean'),
            Condensate_Total_bbl=('Monthly Condensate (bbl)', 'sum'),
            Condensate_Avg_bblpd=('Condensate (bbl/d) (from NAG)', 'mean'),
            Water_Total_bbl=('Monthly Water (bbl)', 'sum'),
            Water_Avg_bwpd=('Water (bwpd)', 'mean'),
            PRODDAYS_sum=('PRODDAYS', 'sum')
        )
    ...
    # -------------------------
    # Monthly & Yearly daily totals (sum of per-well daily rates for each month / year)
    # -------------------------
    monthly_daily_totals = (wf.groupby('Month')
                              .agg(
                                  Oil_bopd_total=('Oil (bopd)', 'sum'),
                                  Gas_mmscfd_total=('Gas (MMscfd)', 'sum'),
                                  Condensate_bblpd_total=('Condensate (bbl/d) (from NAG)', 'sum'),
                                  Water_bwpd_total=('Water (bwpd)', 'sum')
                              ).reset_index())

    monthly_daily_field = (wf.groupby(['Month','Field'])
                              .agg(
                                  Oil_bopd_total=('Oil (bopd)', 'sum'),
                                  Gas_mmscfd_total=('Gas (MMscfd)', 'sum'),
                                  Condensate_bblpd_total=('Condensate (bbl/d) (from NAG)', 'sum'),
    ...
    yearly_daily_totals = (wf.groupby('Year')
                              .agg(
                                  Oil_bopd_total=('Oil (bopd)', 'sum'),
                                  Gas_mmscfd_total=('Gas (MMscfd)', 'sum'),
                                  Condensate_bblpd_total=('Condensate (bbl/d) (from NAG)', 'sum'),
    ...
    yearly_daily_field = (wf.groupby(['Year','Field'])
                              .agg(
                                  Oil_bopd_total=('Oil (bopd)', 'sum'),
                                  Gas_mmscfd_total=('Gas (MMscfd)', 'sum'),
                                  Condensate_bblpd_total=('Condensate (bbl/d) (from NAG)', 'sum'),
    ...
    # pivots by well for plotting & pies (monthly & yearly)
    ...
    outputs = {
        ...
        # new daily totals outputs
        'monthly_daily_totals': monthly_daily_totals,
        'monthly_daily_field': monthly_daily_field,
        'yearly_daily_totals': yearly_daily_totals,
        'yearly_daily_field': yearly_daily_field
    }
    return outputs
...
with tabs[1]:
    ...
    monthly_daily_totals = results.get('monthly_daily_totals', pd.DataFrame())
    monthly_daily_field = results.get('monthly_daily_field', pd.DataFrame())
    ...
    st.markdown("---")
    st.subheader("Monthly total-per-day (all wells combined) — by fluid")
    if monthly_daily_totals.empty:
        st.write("No monthly daily totals available.")
    else:
        st.dataframe(monthly_daily_totals, use_container_width=True)
        st.download_button("Download Monthly Daily Totals (CSV)", monthly_daily_totals.to_csv(index=False).encode('utf-8'), file_name="monthly_daily_totals.csv")

    st.markdown("Daily totals by Field (monthly)")
    if monthly_daily_field.empty:
        st.write("No monthly daily totals by field available.")
    else:
        st.dataframe(monthly_daily_field.head(500), use_container_width=True)
        st.download_button("Download Monthly Daily Totals by Field (CSV)", monthly_daily_field.to_csv(index=False).encode('utf-8'), file_name="monthly_daily_totals_by_field.csv")

    # Plot stacked monthly production (choose fluid)
    ...
    if pivot is None or pivot.empty:
        ...
        selected = st.multiselect("Wells to include (leave empty -> top 10 by total)", wells, default=None)
        if not selected:
            ...
            if not show_all:
                ...
                for idx, y in enumerate(yrs):
                    ...
                    if c > cols:
                        c = 1
                        r += 1
                fig_multi.update_layout(height=300*rows, title_text=f"{fluid_pie} contribution — small multiples")
...
with tabs[4]:
    ...
    st.markdown("---")
    st.subheader("Total production by Field by Year (grouped bars) — choose fluid")
    yearly_field = results.get('yearly_field', pd.DataFrame())
    if yearly_field.empty:
...
st.markdown("---")
st.caption("Notes: GOR expected in scf/bbl (converted to MMscf/bbl). Gas Rate column treated as NAG in MMscfd. CGR assumed bbl/MMscf so condensate = CGR * NAG. Monthly/Yearly totals are computed as sums of monthly volumes; daily totals are sums of per-well daily rates for the period.")
